...
class SpampedeData {
	...
	public void moveSnakeForward(BoardCell cell) {
		BoardCell tail = this.snakeCells.getFirst();
		BoardCell head = this.snakeCells.getLast();
		if (cell.isSpam()) {
			head.becomeBody();
			cell.becomeHead();
  ...
  public BoardCell getNextCellFromBFS() {
    ...
    while(!cellsToSearch.isEmpty()) {
    	...
    	if(current.isSpam()){
    		...
    		for(BoardCell b: this.getNeighbors(current)) {
    			if (b.isOpen() && !((b.inSearchListAlready()))) {
    				cellsToSearch.add(b);
        			b.setParent(current);
        			b.setAddedToSearchList();
    			}
  ...
  private BoardCell getFirstCellInPath(BoardCell start) {
	  if (start.getParent() == this.getSnakeHead()) {
	    	//Base case: when the cell's parent is the snakeHead
	    	return start;
	    }else {
  ...
  public void reverseSnake() {
	...
	// Step 2: reverse the body parts
		...
		for (int i = 0; i < this.snakeCells.size(); i++) {
			newSnake.addFirst(this.snakeCells.get(i));
		}
	...
	// Step 4: calculate the new direction after reversing!
		if (this.getSnakeNeck()==this.getNorthNeighbor(this.getSnakeHead())) {
			this.currentMode = SnakeMode.GOING_SOUTH;
		}
		else if (this.getSnakeNeck()==this.getWestNeighbor(this.getSnakeHead())) {
			this.currentMode = SnakeMode.GOING_EAST;
		}
		else if (this.getSnakeNeck()==this.getSouthNeighbor(this.getSnakeHead())) {
			this.currentMode = SnakeMode.GOING_NORTH;
		}
		else if (this.getSnakeNeck()==this.getEastNeighbor(this.getSnakeHead())) {
			this.currentMode = SnakeMode.GOING_WEST;
		}
  }
